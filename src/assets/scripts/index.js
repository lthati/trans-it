...
var examples = {
  ...
  'love' : {
    list: (function generateLoveList() {
      ...
      // spelling removed.
      var words = ('Liebe,???,Lufu,??,Aimor,Amor,Heyran,????????,???????,?????,?????,??????????,' +
        'Ljubav,Karantez,?????,Láska,Amore,Cariad,Kærlighed,Armastus,???p?,Amo,Amol,Maitasun,' +
        '???,Pyar,Amour,Leafde,Gràdh,?,?,?????,??,???,Ihunanya,Cinta,?????????,Ást,????,' +
        '??????,?????????,????????,Pendo,?????,Milestiba,Meile,Leefde,Bolingo,Szerelem,' +
        '?????,??????,Imhabba,?????,Ái,????,?????,Tlazohtiliztli,Liefde,????,?????,' +
        'Kjærlighet,Kjærleik,?????,Sevgi,????,????,Milosc,Leevde,Dragoste,' +
        'Khuyay,??????,??????,Dashuria,Amuri,????,Ljubezen,Jaceyl,?????????,?????,Rakkaus,' +
        'Kärlek,Pag-ibig,?????,?????,???????,???,Ask,????,Tình yêu,Higugma,????').split(',');
